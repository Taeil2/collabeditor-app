...
export default function Document() {
  ...
  useEffect(() => {
    ...
    if (ghostDivRef.current.children.length === 1) { // cursor at single location
  ...
  const setCursor = () => {
    if ( // cursor is at 0
    ...
    } else if (selection.current[0] === selection.current[1]) { // cursor is at a single position
      const preCharacter = value.substring(0, selection.current[0] - 1)
      const character = value.substring(selection.current[0] - 1, selection.current[0])
      ...
      const lastCharacter = value.substring(selection.current[1] - 1, selection.current[1])
    ...
    } else { // selection is highlighted from somewhere to somewhere
      const preSelection = value.substring(0, selection.current[0] - 1)
      const firstCharacter = value.substring(selection.current[0] - 1, selection.current[0])
      const midSelection =  value.substring(selection.current[0], selection.current[1] - 1)
      const lastCharacter = value.substring(selection.current[1] - 1, selection.current[1])
      const postSelection = value.substring(selection.current[1], value.length)
